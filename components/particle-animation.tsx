...
export default function ParticlesCursorAnimation() {
  ...
  const [glowImageLoaded, setGlowImageLoaded] = useState(false);
  const glowImage = useMemo(() => {
    const img = new Image();
    img.onload = () => {
      console.log("Glow image loaded successfully");
      setGlowImageLoaded(true);
    };
    img.onerror = () => {
  ...
  const pictureTexture = useMemo(() => {
    ...
    return loader.load("/compu Background Removed.png");
  }, []);
  ...
  useFrame((state) => {
    ...
    if (glowImageLoaded && glowImage.complete && glowImage.naturalWidth > 0) {
